...
class Map():
    ...
    def dijkstra(self, edeges: List[List[int]]) -> None:
        ...
        while min_heap:
            ...
            if current_dist > min_cost[node]:
                continue

            for nei, weight in graph[node]:
                new_dist = weight + current_dist
                if new_dist < min_cost.get(nei, float('inf')):
                    min_cost[nei] = new_dist
                    came_from[nei] = node
                    heapq.heappush(min_heap, (new_dist, nei))
        
        # print(came_from)
        self.reconstruct_path(came_from, self.start_hub.name, self.end_hub.name)
